...
public class NewRunMotionProfile extends CommandBase {
  ...
  public NewRunMotionProfile(Drivetrain driveTrain, RobotOdometry odometry, double initialVelocity,
    ...
    setup(driveTrain, odometry, null, null, endVelocity, reversed, false, extraConstraints);
  ...
  public NewRunMotionProfile(Drivetrain driveTrain, RobotOdometry odometry, Pose2d initialPosition,
    ...
    setup(driveTrain, odometry, convertTranslationListToMeters(intermediatePoints),
        endPosition, endVelocity, reversed, false, extraConstraints);
    dynamicTrajectory = false;
    relativeTrajectory = relative;
    startGeneration(initialPosition, initialVelocity);
  }
  ...
  public NewRunMotionProfile(Drivetrain driveTrain, RobotOdometry odometry, List<Translation2d> intermediatePoints,
    ...
    updateConstants();
    setup(driveTrain, odometry, convertTranslationListToMeters(intermediatePoints),
        endPosition, endVelocity, reversed, relative, extraConstraints);
  }
  ...
  private void updateConstants() {
    ...
    */
    ...
    kV = DriveConstants.kvVoltSecondsPerMeter;
    kA = DriveConstants.kaVoltSecondsSquaredPerMeter;
    trackWidth = DriveConstants.kTrackwidthMeters;
    maxVelocity = AutoConstants.kMaxSpeedMetersPerSecond;
    maxAcceleration = AutoConstants.kMaxAccelerationMetersPerSecondSquared;
    maxCentripetalAcceleration = AutoConstants.kMaxCentripetalAcceleration; // 90% of kMaxAccelerationMetersPerSecondSquared  by default
  }
  ...
  public void startGeneration() {
    if (dynamicTrajectory) {
      startGeneration(getCurrentPoseMeters(),
          (driveTrain.getLeftVelocityMeter() + driveTrain.getRightVelocityMeter()) / 2);
    }
  }
    ...
    if (followerCommand == null) {
      ...
      followerCommand = new RamseteCommand(trajectory, this::getCurrentPoseMeters,
          new RamseteController(kRamseteB, kRamseteZeta), new SimpleMotorFeedforward(
            ...
            DriveConstants.kaVoltSecondsSquaredPerMeter), driveKinematics, driveTrain::getWheelSpeeds,
